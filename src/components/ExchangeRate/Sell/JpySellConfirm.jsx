...
const JpySellConfirm = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const { jpyAmount, targetRate } = location.state; // Correct state names

  const finalAmount = (parseFloat(jpyAmount) * parseFloat(targetRate)).toFixed(
    2
  );
  ...
  return (
    <div style={styles.container}>
      ...
      <p style={styles.confirmText}>
        ...
        <br />
        <span style={styles.highlight}>{jpyAmount} ¥</span>을{" "}
        <span style={styles.highlight}>{finalAmount}원</span>으로
        <br />
        바꾸시겠습니까?
      </p>
